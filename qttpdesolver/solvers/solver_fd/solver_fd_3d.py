...
class SolverFD_3d(Solver):
    ...
    def _gen_coefficients(self, PDE, GRD, d, n, h, dim, mode, tau, verb):
        self.Kx = PDE.Kx.build([GRD.xc, GRD.yr, GRD.zr], verb=verb, to_diag=True)
        self.Ky = PDE.Ky.build([GRD.xr, GRD.yc, GRD.zr], verb=verb, to_diag=True)
        self.Kz = PDE.Kz.build([GRD.xr, GRD.yr, GRD.zc], verb=verb, to_diag=True)
