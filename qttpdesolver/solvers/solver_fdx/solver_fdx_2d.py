...
class SolverFDX_2d(Solver):
    ...
    def _gen_system(self, d, n, h, dim, mode, tau, verb):           
        self.Ax  = self.Sx.dot(self.iBx.T).dot(self.Kx).dot(self.iBx).dot(self.Sx) + self.Zx
        self.Ay  = self.Sy.dot(self.iBy.T).dot(self.Ky).dot(self.iBy).dot(self.Sy) + self.Zy
